...
def test_lint_good_example(capfd, get_template_linter) -> None:
    ...
    for linter in get_template_linter:
        test_linter: TemplateLinter = linter(
            str(os.path.abspath(os.path.dirname(__file__))) + "/lint_test_files/lint_good_file"
        )
        ...
        test_linter.check_version_match(
            f"{test_linter.path}/lint_good_test_file", "1.0.0-SNAPSHOT", "bumpversion_files_whitelisted"
        )
...
def test_lint_bad_example(capfd, get_template_linter) -> None:
    ...
    for linter in get_template_linter:
        ...
        test_linter.check_no_cookiecutter_strings()
        test_linter.check_version_match(
            f"{test_linter.path}/lint_bad_test_file", "1.0.0", "bumpversion_files_whitelisted"
        )
        test_linter.print_results()
