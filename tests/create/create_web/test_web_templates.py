...
def deployment_script_tests(path):
    return [Path(f"{path}/deployment_scripts/projectname"),Path(f"{path}/deployment_scripts/projectname.service"),Path(f"{path}/deployment_scripts/README.md"),
            Path(f"{path}/deployment_scripts/setup.sh")]


def website_front_end_tests(path):
    return [Path(f"{path}/projectname/handlers"),Path(f"{path}/projectname/static"),Path(f"{path}/projectname/templates"),
            Path(f"{path}/projectname/translations"),Path(f"{path}/projectname/app.py"),Path(f"{path}/projectname/config.py")
...
def posix_path_super_dir(path):
    return [Path(f"{path}/MANIFEST.in"),Path(f"{path}/.editorconfig"),Path(f"{path}/Makefile"),Path(f"{path}/README.rst"),
            Path(f"{path}/tests"),Path(f"{path}/readthedocs.yml"),Path(f"{path}/babel.cfg"),Path(f"{path}/requirements_dev.txt"),
            ...
            Path(f"{path}/deployment_scripts"),Path(f"{path}/requirements.txt"),Path(f"{path}/setup.cfg"),Path(f"{path}/CHANGELOG.rst"),
...
def test_choose_domain_web_website_flask(monkeypatch,valid_domains,tmp_path):
    ...
    assert (sorted(list(tmp_path.iterdir())) == sorted(posix_path_super_dir(tmp_path)) and sorted(list(Path(tmp_path/"docs").iterdir())) == sorted(docs_subdir(tmp_path))
            ...
            sorted(list(Path(tmp_path/"projectname").iterdir())) == sorted(website_front_end_tests(tmp_path)) and
            sorted(list(Path(tmp_path/"deployment_scripts").iterdir())) == sorted(deployment_script_tests(tmp_path)))
