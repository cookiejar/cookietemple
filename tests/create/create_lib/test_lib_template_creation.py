...
def test_create_lib_project(mocker) -> None:
    """
    Test the creation of a whole C++ lib template without GitHub Repo creation!
    """
    runner = CliRunner()
    ...
    with runner.isolated_filesystem():
        ...
        result = runner.invoke(create, input='lib\ncpp\nhomersimpson\nsimpson@gmail.com\nhomergithub\nn\nexplodeit'
                                             '\ndescription\n1.0.0\nMIT\nn')
        project_path = Path('explodeit').resolve()
        lib_linter_instance = LibCppLint(project_path)

        # lint the created project to ensure everything worked fine
        lib_linter_instance.lint_project(lib_linter_instance, custom_check_files=False, is_subclass_calling=False)

        assert result.exit_code == 0 and matching_project_structure(project_path) and len(lib_linter_instance.failed) == 0
